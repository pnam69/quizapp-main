...
class GamificationService
{
    ...
    public function awardQuizCompletionXp(User $user, int $score, int $totalQuestions, string $difficulty = 'medium'): void
    {
        ...
        DB::transaction(function () use ($user, $baseXp, $score, $totalQuestions) {
            ...
            $userXp = $user->userXp ?? new UserXp(['user_id' => $user->id, 'total_xp' => 0, 'current_level' => 1]);

            // Award XP
            $userXp->total_xp += $baseXp;
            $userXp->current_level = $this->calculateLevel($userXp->total_xp);
            $userXp->save();
            ...
            XpTransaction::create([
                ...
                'metadata' => json_encode([
                    ...
                    'accuracy' => $totalQuestions > 0 ? round(($score / $totalQuestions) * 100, 2) : 0
                ])
            ]);
            ...
            switch ($badge->type) {
                ...
                case 'xp_level':
                    ...
                    if (isset($criteria['level'])) {
                        $userXp = $user->userXp;
                        $unlocked = $userXp && $userXp->current_level >= $criteria['level'];
                    }
                    break;
