    ...
    public static function table(Table $table): Table
    {
        return $table
            ...
            ->columns([
                ...
                Tables\Columns\TextColumn::make('student_assessment_count')
                    ->label('Total Assessments')
                    ->getStateUsing(function ($record) {
                        return AssessmentAttempt::where('user_id', $record->user_id)
                    ...
                    ->sortable(query: function (Builder $query, string $direction) {
                        return $query->orderBy(
                            AssessmentAttempt::selectRaw('COUNT(*)')
                                ->whereColumn('assessment_attempts.user_id', 'assessment_attempts.user_id')
                                ->where('status', 'completed'),
                            $direction
                        );
                    })
                ...
                Tables\Columns\IconColumn::make('passed')
                    ...
                    ->falseColor('danger')
                    ->tooltip(
                        fn($record) => $record->passed
                            ? "Passed: Score ({$record->percentage}%) ≥ Passing Score ({$record->assessment->passing_score}%)"
                            : "Failed: Score ({$record->percentage}%) < Passing Score ({$record->assessment->passing_score}%)"
                    ),
            ...
            ->filters([
                ...
                Tables\Filters\Filter::make('needs_improvement')
                    ...
                    ->query(fn(Builder $query) => $query->where('percentage', '>=', 50)->where('percentage', '<', 70)),
