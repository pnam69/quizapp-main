    ...
    public function getTabs(): array
    {
        return [
            ...
            'excellent' => Tab::make('Excellent (≥80%)')
                ...
                ->badge(fn() => $this->getModel()::query()->where('status', 'completed')->where('percentage', '>=', 80)->count())
                ->badgeColor('success')
                ->modifyQueryUsing(fn(Builder $query) => $query->where('percentage', '>=', 80)),

            'needs_improvement' => Tab::make('Needs Improvement (<60%)')
                ->icon('heroicon-o-exclamation-triangle')
                ->badge(fn() => $this->getModel()::query()->where('status', 'completed')->where('percentage', '<', 60)->count())
                ->badgeColor('warning')
                ->modifyQueryUsing(fn(Builder $query) => $query->where('percentage', '<', 60)),
        ];
    }
}
