<x-filament-panels::page>
    ...
    <div class="fixed inset-0 overflow-hidden pointer-events-none">
        ...
        <div class="absolute top-40 right-16 w-24 h-24 bg-gradient-to-br from-purple-400/10 to-pink-500/10 rounded-full blur-lg animate-pulse delay-1000"></div>
        <div class="absolute bottom-32 left-20 w-40 h-40 bg-gradient-to-br from-green-400/10 to-teal-500/10 rounded-full blur-2xl animate-pulse delay-500"></div>
        <div class="absolute bottom-20 right-32 w-28 h-28 bg-gradient-to-br from-orange-400/10 to-red-500/10 rounded-full blur-xl animate-pulse delay-1500"></div>
    ...
    <div class="relative min-h-screen">
        ...
        <div class="fixed top-24 right-6 z-10 hidden lg:block">
            <div class="bg-white/90 dark:bg-gray-800/90 backdrop-blur-sm rounded-xl shadow-lg p-4 border border-gray-200/50 dark:border-gray-700/50">
                <div class="text-sm font-medium text-gray-900 dark:text-gray-100 mb-2">Progress</div>
                <div class="flex items-center gap-2">
                    <div class="flex-1 bg-gray-200 dark:bg-gray-700 rounded-full h-2">
                        <div class="bg-gradient-to-r from-blue-500 to-indigo-600 h-2 rounded-full transition-all duration-300"
                             style="width: {{ empty($results) ? 0 : (count($results) / count($questions)) * 100 }}%"></div>
                    </div>
                    <span class="text-xs text-gray-600 dark:text-gray-400 min-w-[3rem]">
                        {{ count($results ?? []) }}/{{ count($questions) }}
                    </span>
                </div>
                @if (!empty($results))
                <div class="mt-2 text-xs text-gray-600 dark:text-gray-400">
                    {{ collect($results)->where('isCorrect', true)->count() }} correct
                </div>
                @endif
            </div>
        </div>
