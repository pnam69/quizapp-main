        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <x-filament::card>
                <div class="flex items-center justify-between">
                    <div>
                        <h3 class="text-sm font-medium text-gray-600 dark:text-gray-400">Total Quizzes</h3>
                        <p class="text-2xl font-bold text-primary-600 dark:text-primary-400">{{ $totalQuizzes }}</p>
                    </div>
            ...
            <x-filament::card>
                <div class="flex items-center justify-between">
                    <div>
                        <h3 class="text-sm font-medium text-gray-600 dark:text-gray-400">Completed Quizzes</h3>
                        <p class="text-2xl font-bold text-success-600 dark:text-success-400">{{ $completedQuizzes }}</p>
                    </div>
            ...
            <x-filament::card>
                ...
                @if($recentResults->isEmpty())
                    <div class="text-center py-8">
                        ...
                        <p class="mt-2 text-gray-500">No completed quizzes yet.</p>
                    </div>
                @else
                    <div class="space-y-3">
                        ...
                        <div class="flex justify-between items-center p-3 bg-gray-50 dark:bg-gray-800 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 transition">
                            <div>
                                <p class="font-medium">{{ $quiz->title ?? 'Untitled Quiz' }}</p>
                                <p class="text-xs text-gray-500">{{ $quiz->updated_at->diffForHumans() }}</p>
                            </div>
                            <span class="px-3 py-1 rounded-full text-sm font-semibold
                                @if(($quiz->score ?? 0) >= 70) bg-success-100 text-success-700 dark:bg-success-900 dark:text-success-300
                                @elseif(($quiz->score ?? 0) >= 50) bg-warning-100 text-warning-700 dark:bg-warning-900 dark:text-warning-300
            ...
            <x-filament::card>
                ...
                @if($nextQuiz)
                    <div class="p-4 bg-primary-50 dark:bg-primary-900/20 rounded-lg border border-primary-200 dark:border-primary-800">
                        <p class="font-medium text-lg mb-2">{{ $nextQuiz->title ?? 'Untitled Quiz' }}</p>
                        <p class="text-sm text-gray-600 dark:text-gray-400 mb-4">
                            Created {{ $nextQuiz->created_at->diffForHumans() }}
                        </p>
                        <x-filament::button
                            tag="a"
                            color="primary"
                ...
                @else
                    <div class="text-center py-8">
                        ...
                        <p class="mt-2 text-gray-500">You have no pending quizzes.</p>
