        ...
        <div class="bg-gradient-to-r from-green-500 to-emerald-600 dark:from-green-600 dark:to-emerald-700 rounded-xl shadow-lg p-8 text-white mb-6">
            ...
            <div class="flex gap-4 text-sm mt-4">
                <div class="bg-white/10 backdrop-blur-sm rounded-lg px-3 py-2">
                    ✓{{ $assessmentAttempts->count() }} Assessment{{ $assessmentAttempts->count() !== 1 ? 's' : '' }} Completed
                </div>
                @if($assessmentAttempts->count() > 0)
                <div class="bg-white/10 backdrop-blur-sm rounded-lg px-3 py-2">
                    📊 Average Score: {{ round($assessmentAttempts->avg('percentage'), 1) }}%
                </div>
                @endif
            </div>
        </div>
        ...
        <div class="grid md:grid-cols-4 gap-4 mb-6">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 border-2 border-green-200 dark:border-green-800">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <div class="text-2xl font-bold text-green-900 dark:text-green-100">{{ round($assessmentAttempts->avg('percentage'), 1) }}%</div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 border-2 border-purple-200 dark:border-purple-800">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <div class="text-2xl font-bold text-purple-900 dark:text-purple-100">{{ round($assessmentAttempts->max('percentage'), 1) }}%</div>
                            ...
                            <div class="flex-1">
                                ...
                                <div class="flex flex-wrap gap-3 mt-3">
                                    <div class="inline-flex items-center gap-1.5 bg-{{ $scoreColor }}-100 dark:bg-{{ $scoreColor }}-900/30 text-{{ $scoreColor }}-700 dark:text-{{ $scoreColor }}-300 px-3 py-1 rounded-full text-sm font-bold">
                                        ...
                                        Score: {{ $attempt->points_earned }}/{{ $attempt->total_points }} ({{ number_format($percentage, 1) }}%)
                                    </div>
                                    @if($attempt->passed !== null)
                                    <div class="inline-flex items-center gap-1.5 bg-{{ $passed ? 'green' : 'red' }}-100 dark:bg-{{ $passed ? 'green' : 'red' }}-900/30 text-{{ $passed ? 'green' : 'red' }}-700 dark:text-{{ $passed ? 'green' : 'red' }}-300 px-3 py-1 rounded-full text-sm font-bold">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6">
                ...
                <div class="grid md:grid-cols-3 gap-4 mb-6">
                    <div class="bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 rounded-xl p-6">
                        ...
                        <div class="text-3xl font-bold text-blue-900 dark:text-blue-100">{{ $selectedAttempt->points_earned }}/{{ $selectedAttempt->total_points }}</div>
                        <div class="text-sm text-blue-700 dark:text-blue-300 mt-1">{{ number_format($percentage, 1) }}%</div>
                    </div>
