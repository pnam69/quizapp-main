<x-filament::page>
    ...
    <div class="fixed inset-0 overflow-hidden pointer-events-none">
        ...
        <div class="absolute top-16 left-8 w-36 h-36 bg-gradient-to-br from-green-400/10 to-teal-500/10 rounded-full blur-xl animate-pulse"></div>
        <div class="absolute top-32 right-12 w-28 h-28 bg-gradient-to-br from-blue-400/10 to-indigo-500/10 rounded-full blur-lg animate-pulse delay-1000"></div>
        <div class="absolute bottom-40 left-16 w-44 h-44 bg-gradient-to-br from-purple-400/10 to-pink-500/10 rounded-full blur-2xl animate-pulse delay-500"></div>
        <div class="absolute bottom-24 right-20 w-32 h-32 bg-gradient-to-br from-orange-400/10 to-yellow-500/10 rounded-full blur-xl animate-pulse delay-1500"></div>

        {{-- Subtle grid pattern --}}
        <div class="absolute inset-0 opacity-[0.015] dark:opacity-[0.03]">
            <div class="absolute inset-0" style="background-image: radial-gradient(circle at 1px 1px, rgba(16, 185, 129, 0.3) 1px, transparent 0); background-size: 24px 24px;"></div>
        </div>

        {{-- Corner decorations --}}
        <div class="absolute top-0 left-0 w-80 h-80 bg-gradient-to-br from-green-500/5 to-transparent rounded-br-full"></div>
        <div class="absolute top-0 right-0 w-80 h-80 bg-gradient-to-bl from-teal-500/5 to-transparent rounded-bl-full"></div>
        <div class="absolute bottom-0 left-0 w-80 h-80 bg-gradient-to-tr from-blue-500/5 to-transparent rounded-tr-full"></div>
        <div class="absolute bottom-0 right-0 w-80 h-80 bg-gradient-to-tl from-purple-500/5 to-transparent rounded-tl-full"></div>
    </div>

    <div class="relative min-h-screen">
        {{-- Study stats sidebar --}}
        <div class="fixed top-24 right-6 z-10 hidden lg:block">
            <div class="bg-white/90 dark:bg-gray-800/90 backdrop-blur-sm rounded-xl shadow-lg p-4 border border-gray-200/50 dark:border-gray-700/50">
                <div class="flex items-center gap-2 mb-3">
                    ...
                    <span class="text-sm font-medium text-gray-900 dark:text-gray-100">Study Stats</span>
                </div>
                <div class="space-y-3">
                    <div class="flex items-center justify-between">
                        <span class="text-xs text-gray-600 dark:text-gray-400">Resources</span>
                        <span class="text-sm font-semibold text-gray-900 dark:text-gray-100">{{ count($this->hubs) }}</span>
                    </div>
                    <div class="flex items-center justify-between">
                        <span class="text-xs text-gray-600 dark:text-gray-400">Videos</span>
                        <span class="text-sm font-semibold text-gray-900 dark:text-gray-100">{{ collect($this->hubs)->where('type', 'video')->count() }}</span>
                    </div>
                    <div class="flex items-center justify-between">
                        <span class="text-xs text-gray-600 dark:text-gray-400">Documents</span>
                        <span class="text-sm font-semibold text-gray-900 dark:text-gray-100">{{ collect($this->hubs)->where('type', 'document')->count() }}</span>
                    </div>
                    <div class="flex items-center justify-between">
                        <span class="text-xs text-gray-600 dark:text-gray-400">Links</span>
                        <span class="text-sm font-semibold text-gray-900 dark:text-gray-100">{{ collect($this->hubs)->where('type', 'link')->count() }}</span>
                    </div>
                </div>
            </div>
        </div>
