<x-filament-panels::page wire:poll>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg overflow-hidden">
            ...
            <div class="p-8">
                ...
                <form wire:submit.prevent="submit" class="space-y-6">
                    ...
                    $letters = ['A', 'B', 'C', 'D', 'E', 'F'];
                    @endphp
                    <div class="border-2 rounded-xl overflow-hidden {{ $isAnswered ? ($results[$question->id]['isCorrect'] ? 'border-green-300 dark:border-green-700' : 'border-red-300 dark:border-red-700') : 'border-gray-200 dark:border-gray-700' }}">
                        <div class="p-6 {{ $isAnswered ? ($results[$question->id]['isCorrect'] ? 'bg-green-50 dark:bg-green-900/20' : 'bg-red-50 dark:bg-red-900/20') : 'bg-white dark:bg-gray-800' }}">
                            <div class="flex items-start gap-3 mb-4">
                                ...
                                @if ($isAnswered)
                                <span class="shrink-0 px-3 py-1 rounded-full text-sm font-medium {{ $results[$question->id]['isCorrect'] ? 'bg-green-200 dark:bg-green-800 text-green-800 dark:text-green-200' : 'bg-red-200 dark:bg-red-800 text-red-800 dark:text-red-200' }}">
                            ...
                            <div class="space-y-3">
                                ...
                                @php
                                $isCorrectOption = $option->id == $correct;
                                $isChosenOption = $option->id == $chosen;
                                $isCurrentlySelected = !$isAnswered && isset($answers[$question->id]) && $answers[$question->id] == $option->id;
                                $optionClass = '';
                                if ($isAnswered) {
                                if ($isCorrectOption) {
                                $optionClass = 'border-green-500 bg-green-100 dark:bg-green-900/40';
                                } elseif ($isChosenOption) {
                                $optionClass = 'border-red-500 bg-red-100 dark:bg-red-900/40';
                                }
                                } elseif ($isCurrentlySelected) {
                                $optionClass = 'border-blue-500 bg-blue-50 dark:bg-blue-900/30 shadow-md';
                                }
                                @endphp
                                <label class="flex items-start p-4 border-2 rounded-xl cursor-pointer transition-all duration-200 group
                                    {{ $optionClass ?: 'border-gray-200 dark:border-gray-700 hover:border-blue-300 dark:hover:border-blue-600 hover:shadow-sm' }}">
                                    <div class="flex items-center gap-4 w-full">
                                        <div class="shrink-0 w-10 h-10 rounded-full border-2 flex items-center justify-center font-bold text-sm
                                            {{ $isCorrectOption ? 'border-green-500 bg-green-500 text-white' : ($isChosenOption ? 'border-red-500 bg-red-500 text-white' : ($isCurrentlySelected ? 'border-blue-500 bg-blue-500 text-white' : 'border-gray-300 dark:border-gray-600 text-gray-500 dark:text-gray-400')) }}">
                                        ...
                                        <input type="radio"
                                            ...
                                            class="sr-only"
                                            {{ $isAnswered ? 'disabled' : '' }}>
                                        <span class="flex-1 text-base {{ ($isCorrectOption || $isChosenOption || $isCurrentlySelected) ? 'font-medium text-gray-900 dark:text-gray-100' : 'text-gray-700 dark:text-gray-300' }}">
                                            {{ $option->answer }}
                                        </span>
                                        @if ($isCorrectOption || $isChosenOption)
                                        <svg class="w-6 h-6 shrink-0 {{ $isCorrectOption ? 'text-green-500' : 'text-red-500' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="{{ $isCorrectOption ? 'M5 13l4 4L19 7' : 'M6 18L18 6M6 6l12 12' }}"></path>
                                        </svg>
                                        @elseif ($isCurrentlySelected)
