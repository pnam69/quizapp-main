                ...
                <div class="text-center">
                    <div class="relative inline-flex items-center justify-center">
                        <svg class="w-24 h-24 transform -rotate-90" viewBox="0 0 36 36">
                            ...
                            <path d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831"
                                  fill="none" stroke="currentColor" stroke-width="2" stroke-dasharray="{{ $record->total_points > 0 ? round(($record->score / $record->total_points) * 100, 1) : 0 }}, 100"
                                  class="{{ $record->total_points > 0 && (($record->score / $record->total_points) * 100) >= 80 ? 'text-green-500' : (($record->total_points > 0 && ($record->score / $record->total_points) * 100) >= 60 ? 'text-yellow-500' : 'text-red-500') }}"/>
                        </svg>
                        <div class="absolute inset-0 flex items-center justify-center">
                            <span class="text-2xl font-bold {{ $record->total_points > 0 && (($record->score / $record->total_points) * 100) >= 80 ? 'text-green-600' : (($record->total_points > 0 && ($record->score / $record->total_points) * 100) >= 60 ? 'text-yellow-600' : 'text-red-600') }}">
