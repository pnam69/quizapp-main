<div class="space-y-8 max-w-4xl mx-auto">
    {{-- Header Section --}}
    <div class="bg-gradient-to-r from-emerald-50 via-blue-50 to-purple-50 dark:from-emerald-900/20 dark:via-blue-900/20 dark:to-purple-900/20 rounded-xl p-6 border border-emerald-200 dark:border-emerald-700 shadow-sm">
        <div class="flex items-center justify-between">
            <div>
                <h2 class="text-2xl font-bold bg-gradient-to-r from-emerald-600 to-blue-600 bg-clip-text text-transparent">{{ $record->user->name }}</h2>
                <p class="text-slate-600 dark:text-slate-400 mt-1">{{ $record->user->email }}</p>
            </div>
            <div class="text-right">
                <div class="text-sm text-purple-600 dark:text-purple-400 font-medium">Assessment</div>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-xl shadow-lg border border-slate-200 dark:border-slate-700 overflow-hidden">
        <div class="bg-gradient-to-r from-emerald-500 via-blue-500 to-purple-500 px-6 py-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div class="text-center">
                    <div class="relative inline-flex items-center justify-center">
                        <svg class="w-24 h-24 transform -rotate-90" viewBox="0 0 36 36">
                            ...
                            <path d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831"
                                fill="none" stroke="currentColor" stroke-width="3" stroke-dasharray="{{ $record->total_points > 0 ? round(($record->score / $record->total_points) * 100, 1) : 0 }}, 100"
                                class="{{ $record->total_points > 0 && (($record->score / $record->total_points) * 100) >= 80 ? 'text-emerald-500' : (($record->total_points > 0 && ($record->score / $record->total_points) * 100) >= 60 ? 'text-amber-500' : 'text-red-500') }}" />
                        </svg>
                        <div class="absolute inset-0 flex items-center justify-center">
                            <span class="text-2xl font-bold {{ $record->total_points > 0 && (($record->score / $record->total_points) * 100) >= 80 ? 'text-emerald-600' : (($record->total_points > 0 && ($record->score / $record->total_points) * 100) >= 60 ? 'text-amber-600' : 'text-red-600') }}">
                    ...
                    <div class="bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 rounded-lg p-4 border border-blue-200 dark:border-blue-700">
                        ...
                        <div class="w-full bg-slate-200 dark:bg-slate-600 rounded-full h-2 mt-2">
                            <div class="bg-gradient-to-r from-blue-500 to-indigo-500 h-2 rounded-full transition-all duration-300"
                                style="width: {{ $record->total_points > 0 ? (($record->score / $record->total_points) * 100) : 0 }}%"></div>
                        </div>
                    </div>
                </div>
                ...
                <div class="text-center">
                    <div class="flex flex-col items-center">
                        @if($record->passed)
                        <div class="w-16 h-16 bg-gradient-to-br from-emerald-100 to-green-100 dark:from-emerald-900/30 dark:to-green-900/30 rounded-full flex items-center justify-center mb-3 border-2 border-emerald-200 dark:border-emerald-700">
                            <svg class="w-8 h-8 text-emerald-600 dark:text-emerald-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                            </svg>
                        </div>
                        <div class="text-lg font-bold bg-gradient-to-r from-emerald-600 to-green-600 bg-clip-text text-transparent">PASSED</div>
                        @else
                        <div class="w-16 h-16 bg-gradient-to-br from-red-100 to-pink-100 dark:from-red-900/30 dark:to-pink-900/30 rounded-full flex items-center justify-center mb-3 border-2 border-red-200 dark:border-red-700">
                            <svg class="w-8 h-8 text-red-600 dark:text-red-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                            </svg>
                        </div>
                        <div class="text-lg font-bold bg-gradient-to-r from-red-600 to-pink-600 bg-clip-text text-transparent">FAILED</div>
                        @endif
                        <p class="text-sm font-medium text-slate-600 dark:text-slate-400 mt-1">Assessment Status</p>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
        <div class="bg-gradient-to-r from-rose-50 via-pink-50 to-purple-50 dark:from-rose-900/20 dark:via-pink-900/20 dark:to-purple-900/20 px-6 py-4 border-b border-slate-200 dark:border-slate-600">
        ...
        <div class="p-6">
            <div class="space-y-4">
                ...
                <div class="bg-gradient-to-r {{ $answer->is_correct ? 'from-emerald-50 to-green-50 dark:from-emerald-900/10 dark:to-green-900/10' : 'from-red-50 to-pink-50 dark:from-red-900/10 dark:to-pink-900/10' }} rounded-lg p-4 border-l-4 {{ $answer->is_correct ? 'border-l-emerald-500' : 'border-l-red-500' }} hover:shadow-md transition-shadow">
                    <div class="flex items-start justify-between">
                        <div class="flex-1">
                            <div class="flex items-center gap-3 mb-2">
                                <span class="flex items-center justify-center w-6 h-6 {{ $answer->is_correct ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-300' : 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-300' }} text-xs font-bold rounded-full">
                                    {{ $index + 1 }}
                                </span>
                            ...
                            <div class="bg-white dark:bg-slate-600 rounded-md p-3 border {{ $answer->is_correct ? 'border-emerald-200 dark:border-emerald-700' : 'border-red-200 dark:border-red-700' }}">
                                <div class="flex items-center gap-2 mb-1">
                                    <div class="w-5 h-5 {{ $answer->is_correct ? 'bg-emerald-100' : 'bg-blue-100' }} rounded-full flex items-center justify-center">
                                        <svg class="w-3 h-3 {{ $answer->is_correct ? 'text-emerald-600' : 'text-blue-600' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                                        </svg>
                                    </div>
                                    <span class="text-xs font-medium {{ $answer->is_correct ? 'text-emerald-700 dark:text-emerald-300' : 'text-blue-700 dark:text-blue-300' }} uppercase tracking-wide">Student's Answer</span>
                                </div>
                        ...
                        <div class="ml-4 flex flex-col items-end">
                            <div class="text-lg font-bold {{ $answer->is_correct ? 'text-emerald-600' : 'text-red-600' }}">
