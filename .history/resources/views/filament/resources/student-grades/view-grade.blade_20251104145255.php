<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
        {{-- Score Card --}}
        <div class="bg-white dark:bg-slate-900 rounded-xl p-5 border-2 {{ $record->percentage >= 70 ? 'border-green-500/20' : ($record->percentage >= 50 ? 'border-yellow-500/20' : 'border-red-500/20') }} shadow-sm">
            <div class="flex items-center justify-between mb-3">
                <div class="w-10 h-10 rounded-lg {{ $record->percentage >= 70 ? 'bg-green-100 dark:bg-green-900/50' : ($record->percentage >= 50 ? 'bg-yellow-100 dark:bg-yellow-900/50' : 'bg-red-100 dark:bg-red-900/50') }} flex items-center justify-center">
        ...
        {{-- Points Card --}}
        <div class="bg-white dark:bg-slate-900 rounded-xl p-5 border-2 border-blue-500/20 shadow-sm">
            <div class="flex items-center justify-between mb-3">
            ...
            <div class="text-3xl font-bold text-slate-900 dark:text-white">
                {{ intval($record->score) }}<span class="text-lg text-slate-500 dark:text-slate-400">/{{ intval($record->total_points) }}</span>
            </div>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-5">
            ...
            <div class="space-y-3">
                <div class="flex items-start justify-between py-2">
                    ...
                    <span class="text-sm font-medium text-slate-900 dark:text-white text-right">{{ $record->user->name }}</span>
                </div>
                <div class="flex items-start justify-between py-2 border-t border-slate-100 dark:border-slate-700">
                    ...
                    <span class="text-sm font-medium text-slate-900 dark:text-white text-right">{{ $record->user->email }}</span>
                </div>
                @if($record->user->classrooms->count() > 0)
                <div class="flex items-start justify-between py-2 border-t border-slate-100 dark:border-slate-700">
                    <span class="text-sm text-slate-600 dark:text-slate-400">Classroom</span>
                    <span class="text-sm font-medium text-slate-900 dark:text-white text-right">{{ $record->user->classrooms->pluck('name')->join(', ') }}</span>
                </div>
                @endif
                @if($record->user->sections->count() > 0)
                <div class="flex items-start justify-between py-2 border-t border-slate-100 dark:border-slate-700">
                    <span class="text-sm text-slate-600 dark:text-slate-400">Faculty</span>
                    <span class="text-sm font-medium text-slate-900 dark:text-white text-right">{{ $record->user->sections->pluck('name')->join(', ') }}</span>
                </div>
                @endif
                @if($record->user->certifications->count() > 0)
                <div class="flex items-start justify-between py-2 border-t border-slate-100 dark:border-slate-700">
                    <span class="text-sm text-slate-600 dark:text-slate-400">Department</span>
                    <span class="text-sm font-medium text-slate-900 dark:text-white text-right">{{ $record->user->certifications->pluck('name')->join(', ') }}</span>
                </div>
                @endif
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-5">
            ...
            <div class="space-y-3">
                <div class="flex items-start justify-between py-2">
                    ...
                    <span class="text-sm font-medium text-slate-900 dark:text-white text-right">{{ $record->assessment->title }}</span>
                </div>
                <div class="flex items-start justify-between py-2 border-t border-slate-100 dark:border-slate-700">
                    <span class="text-sm text-slate-600 dark:text-slate-400">Date</span>
                    <span class="text-sm font-medium text-slate-900 dark:text-white">{{ $record->submitted_at->format('M d, Y') }}</span>
                </div>
                <div class="flex items-start justify-between py-2 border-t border-slate-100 dark:border-slate-700">
                    <span class="text-sm text-slate-600 dark:text-slate-400">Time</span>
                    <span class="text-sm font-medium text-slate-900 dark:text-white">{{ $record->submitted_at->format('h:i A') }}</span>
                </div>
                <div class="flex items-start justify-between py-2 border-t border-slate-100 dark:border-slate-700">
                    <span class="text-sm text-slate-600 dark:text-slate-400">Duration</span>
                    <span class="text-sm font-medium text-slate-900 dark:text-white">
                        @if($record->started_at && $record->submitted_at)
                        {{ $record->started_at->diffInMinutes($record->submitted_at) }} minutes
                        @else
                        Not tracked
                        @endif
                    </span>
                ...
                <div class="flex items-start justify-between py-2 border-t border-slate-100 dark:border-slate-700">
                    ...
                    <span class="text-sm font-medium capitalize text-slate-900 dark:text-white">{{ $record->status }}</span>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden">
        <div class="bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-slate-700 dark:to-slate-700 px-5 py-4 border-b border-slate-200 dark:border-slate-600">
            <div class="flex items-center justify-between">
                ...
                <div class="flex items-center gap-4">
                    <div class="flex items-center gap-2">
                        ...
                        <span class="text-sm text-slate-700 dark:text-slate-300">{{ $record->attemptAnswers->where('is_correct', true)->count() }} Correct</span>
                    </div>
                    <div class="flex items-center gap-2">
                        <div class="w-2 h-2 rounded-full bg-red-500"></div>
                        <span class="text-sm text-slate-700 dark:text-slate-300">{{ $record->attemptAnswers->where('is_correct', false)->count() }} Incorrect</span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="p-5 max-h-[500px] overflow-y-auto">
            <div class="space-y-3">
                ...
                <div class="bg-slate-50 dark:bg-slate-900/50 rounded-xl p-4 border {{ $answer->is_correct ? 'border-green-200 dark:border-green-900/50' : 'border-red-200 dark:border-red-900/50' }}">
                    <div class="flex items-start gap-3">
                        <div class="flex-shrink-0">
                            <div class="w-8 h-8 rounded-lg {{ $answer->is_correct ? 'bg-green-100 dark:bg-green-900/30' : 'bg-red-100 dark:bg-red-900/30' }} flex items-center justify-center">
                                <span class="text-sm font-bold {{ $answer->is_correct ? 'text-green-700 dark:text-green-400' : 'text-red-700 dark:text-red-400' }}">
                                    {{ $index + 1 }}
                                </span>
                            </div>
                        </div>
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="flex items-start justify-between gap-3 mb-2">
                                ...
                                <div class="flex-shrink-0 flex items-center gap-2">
                                    ...
                                    @endif
                                    <span class="text-sm font-bold {{ $answer->is_correct ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400' }}">
