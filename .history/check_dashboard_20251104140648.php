...
$allAssessments = App\Models\Assessment::where('is_published', true)->where('is_active', true)->get();
echo 'All published assessments: ' . $allAssessments->count() . PHP_EOL;
$allAssessments->each(function($a) {
    echo '- ' . $a->title . ' (ID: ' . $a->id . ', Section: ' . ($a->section_id ?? 'NULL') . ', Classroom: ' . ($a->classroom_id ?? 'NULL') . ')' . PHP_EOL;
});
...
// Check all attempts by user 21
$allAttempts = App\Models\AssessmentAttempt::where('user_id', 21)->get();
echo 'All attempts by user 21: ' . $allAttempts->count() . PHP_EOL;
$allAttempts->each(function($attempt) {
    $assessment = $attempt->assessment;
    echo '- Assessment: ' . ($assessment ? $assessment->title : 'Unknown') . ' (ID: ' . $attempt->assessment_id . '), Status: ' . $attempt->status . ', Percentage: ' . ($attempt->percentage ?? 'NULL') . PHP_EOL;
});
...
echo 'Attempts for available assessments: ' . $attempts->count() . PHP_EOL;
