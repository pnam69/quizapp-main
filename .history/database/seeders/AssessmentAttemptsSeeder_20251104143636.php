...
class AssessmentAttemptsSeeder extends Seeder
{
    ...
    private function getStudentPerformanceScore(User $student, Assessment $assessment): float
    {
        ...
        $yearMultiplier = match (true) {
            str_contains($student->email, '1st') => 0.90, // 1st year students: 90% of base
            str_contains($student->email, '2nd') => 1.00, // 2nd year: 100% of base
            str_contains($student->email, '3rd') => 1.10, // 3rd year: 110% of base (best)
            default => 0.95
        };
        ...
        // Base score between 55-95, adjusted by multipliers (higher range for better distribution)
        $baseScore = rand(55, 95) * $yearMultiplier * $difficultyMultiplier * $categoryBonus;

        // Ensure score is within reasonable bounds
        return max(35, min(100, $baseScore));
    }
