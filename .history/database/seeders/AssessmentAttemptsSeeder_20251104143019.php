...
class AssessmentAttemptsSeeder extends Seeder
{
    ...
    private function getStudentPerformanceScore(User $student, Assessment $assessment): float
    {
        ...
        $yearMultiplier = match(true) {
            str_contains($student->email, '1st') => 0.75, // 1st year students perform worse
            str_contains($student->email, '2nd') => 0.85, // 2nd year better
            str_contains($student->email, '3rd') => 0.95, // 3rd year best
            default => 0.80
        };
        ...
        // Base score between 40-95, adjusted by multipliers
        $baseScore = rand(40, 95) * $yearMultiplier * $difficultyMultiplier * $categoryBonus;

        // Ensure score is within reasonable bounds
        return max(10, min(100, $baseScore));
    }
    ...
    private function getCategoryBonus(User $student, string $category): float
    {
        ...
        if (isset($studentCategories[$studentKey]) && in_array($category, $studentCategories[$studentKey])) {
            return 1.15; // 15% bonus for strong categories
        }
    ...
    {
        // First attempts might be lower, subsequent attempts improve
        $adjustment = match($attemptNumber) {
            1 => rand(-10, 5),   // First attempt: -10 to +5
            2 => rand(-5, 10),   // Second attempt: -5 to +10
            3 => rand(0, 15),    // Third attempt: 0 to +15
            default => 0
        };

        return max(10, min(100, $baseScore + $adjustment));
    }
    ...
    private function generateMockAnswers(Assessment $assessment): array
    {
        ...
        foreach ($questions as $question) {
            ...
            if ($options->isNotEmpty()) {
                ...
                $selectedOption = rand(0, 1) ? $correctOption : $options->random();
