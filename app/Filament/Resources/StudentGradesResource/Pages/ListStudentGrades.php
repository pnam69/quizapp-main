...
class ListStudentGrades extends ListRecords
{
    ...
    public function getTabs(): array
    {
        return [
            ...
            'excellent' => Tab::make('Excellent (≥80%)')
                ->icon('heroicon-o-star')
                ->badge(fn() => $this->getModel()::query()->where('status', 'completed')->whereRaw('(score / total_points) * 100 >= 80')->count())
                ->badgeColor('success')
                ->modifyQueryUsing(fn(Builder $query) => $query->whereRaw('(score / total_points) * 100 >= 80')),

            'needs_improvement' => Tab::make('Needs Improvement (<60%)')
                ->icon('heroicon-o-exclamation-triangle')
                ->badge(fn() => $this->getModel()::query()->where('status', 'completed')->whereRaw('(score / total_points) * 100 < 60')->count())
                ->badgeColor('warning')
                ->modifyQueryUsing(fn(Builder $query) => $query->whereRaw('(score / total_points) * 100 < 60')),
        ];
    }
}
