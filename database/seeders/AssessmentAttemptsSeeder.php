...
class AssessmentAttemptsSeeder extends Seeder
{
    public function run(): void
    {
        ...
        $students = User::where('is_admin', 0)->where('is_active', 1)->with(['classrooms', 'sections'])->get();
        ...
        // Group students by their classrooms to make assessment assignment more realistic
        $classroomGroups = $students->groupBy(function ($student) {
            return $student->classrooms->first()?->id ?? 'no_classroom';
        });

        foreach ($classroomGroups as $classroomId => $classroomStudents) {
            // Each classroom gets 2-4 random assessments to attempt
            $classroomAssessments = $assessments->random(min(4, $assessments->count()));

            foreach ($classroomAssessments as $assessment) {
                $this->command->info("Creating attempts for assessment: {$assessment->title} (Classroom: {$classroomId})");

                // 60-80% of students in this classroom attempt each assessment
                $attemptingStudents = $classroomStudents->random(
                    (int) ($classroomStudents->count() * (0.6 + rand(0, 20) / 100))
                );

                foreach ($attemptingStudents as $student) {
                    // Create 1-2 attempts per student per assessment (most don't retake)
    ...
    private function createAssessmentAttempt(Assessment $assessment, User $student, int $attemptNumber)
    {
        ...
        // Generate answers based on the calculated score
        $answers = $this->generateMockAnswers($assessment, $attemptScore);

        // Calculate points earned from the answers
        $pointsEarned = collect($answers)->sum('points_earned');
        ...
        AssessmentAttempt::create([
            ...
            'score' => $percentage, // Use calculated percentage
        ...
        $this->command->info("  Created attempt #{$attemptNumber} for {$student->name}: {$percentage}% (" . ($passed ? 'PASSED' : 'FAILED') . ")");
    }
    ...
    private function generateMockAnswers(Assessment $assessment, float $targetScore): array
    {
        // Generate mock answers array for the attempt based on target score
        $answers = [];
        $questions = $assessment->questions;
        $totalQuestions = $questions->count();
        $correctAnswersNeeded = round(($targetScore / 100) * $totalQuestions);
        ...
        foreach ($shuffledQuestions as $question) {
            ...
            if ($options->isNotEmpty()) {
                ...
                // Decide if this answer should be correct based on target score
                $shouldBeCorrect = $correctCount < $correctAnswersNeeded;
                ...
                if ($shouldBeCorrect && $correctOption) {
                    ...
                    // Student got it wrong - select a random incorrect option
                    $incorrectOptions = $options->where('is_correct', false);
                    $selectedOption = $incorrectOptions->isNotEmpty() ? $incorrectOptions->random() : $options->random();
                }
